...
export default function Started({ getStarted }) {
  const [fallbackImage, setFallbackImage] = useState(
    "/images/grow-your-brand.webp"
  );

  useEffect(() => {
    if (getStarted?.image?.data?.attributes?.url)
      setFallbackImage(
        `${process.env.NEXT_PUBLIC_API_URL}${getStarted.image.data.attributes.url}`
      );
  }, [getStarted?.image?.data?.attributes?.url]);
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              <div className="flex flex-col justify-between text-center lg:text-left gap-4">
                <div>
                  ...
                  <h2>{getStarted?.title || "Ready to Grow Your Brand?"}</h2>
                  <span>
                ...
                <div>
                  <Link
                    href="/#pricing-plan"
                    className="btn btn-primary min-w-52 px-5 h-12">
              ...
              <div>
                <Image
                  src={fallbackImage}
                  onError={() =>
                    setFallbackImage("/images/grow-your-brand.webp")
                  }
                  width={800}
                  height={470}
