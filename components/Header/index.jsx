...
const Header = ({ plan }) => {
	...
	useEffect(() => {
		const fetchPackages = async () => {
			try {
				const res = await fetch(
					`${process.env.NEXT_PUBLIC_API_URL}/api/publishing-networks?populate=deep`
				)
				const data = await res.json()

				const filteredPackages = data?.data?.filter(
					(item) => !item.attributes?.name?.toLowerCase()?.includes('test')
				)
		...
		if (!plan) fetchPackages()
		else setPackageList(plan?.pricingPlan?.plans?.data || [])
	}, [])
                  ...
                  Reviews
						...
						<div className=' menu-button'>
							<div className='w-full lg:w-fit'>
								<button
									className='rounded-full btn btn-outline btn-sm btn-main'
									onClick={() =>
							...
							<div className='w-full lg:w-fit'>
								<Link
									href='/#pricing-plan'
									className='rounded-full btn btn-primary btn-sm btn-main'
								>
									Order Now
									<Icon icon='bi:arrow-right' height={24} />
								</Link>
